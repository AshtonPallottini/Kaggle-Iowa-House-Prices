---
title: "Final Code"
author: "Ashton Pallottini, Carson Fick, Ben Wurst, Zongheng Zhang"
...
```{r}
#Data Exploration
cor(all.df$OverallCond, all.df$OverallQual)
cor(all.df$ExterCond, all.df$ExterQual)
cor(all.df$BsmtCond, all.df$BsmtQual)

cor(all.df$LotFrontage, all.df$LotArea)
cor(all.df$BldgType, all.df$HouseStyle)


cor(all.df$Exterior1st, all.df$Exterior2nd)
```
...
which.min(pcr.cv.rmse$val[1,1,-1])
...
which.min(pls.cv.rmse$val[1,1,-1])
...
tree.pred <- predict(tree.fit, newdata = all.df.nolvg[1458:2916,-77])
tree.pred <- unname(tree.pred)
tree.pred <- e^tree.pred
tree.submit <- data.frame(test.ids,tree.pred)
...
colnames(tree.submit) <- c("Id", "SalePrice")
#write.csv(tree.submit, "tree1.csv")
```
